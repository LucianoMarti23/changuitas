...
<x-page>
    ...
    <body class="h-screen flex items-center justify-center">
        ...
        <h1
            style="font-family: 'Inter', sans-serif; text-align: center; color: #4A90E2; font-size: 3em; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">
                    ...
                    <div class="relative h-full w-full flex items-center justify-center">
                        <div
                            ...
                            <button
                                ...
                                <div
                                    class="w-full h-full flex items-center justify-center p-2 rounded-full active:scale-95 hover:scale-95">
                            ...
                            <button
                                ...
                                <span
                                    ...
                                    <svg class="w-8 h-8 text-dark-800 dark:text-primary-600"
                            ...
                            <!-- El Modal -->
                            <div id="myModal"
                                class="modal fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 hidden">
                                <div class="modal-content bg-white p-6 rounded-lg w-[90%] max-w-md relative">
                                    <!-- Botón para cerrar el modal -->
                                    <span id="closeModal"
                                        class="absolute top-2 right-2 text-2xl cursor-pointer text-dark-700">×</span>
                            ...
                            <button
                                ...
                                <span
                                    class="block w-[40px] h-[40px] transition-all duration-500 rounded-full z-[2] bg-light-200 p-1">
                                    <svg class="w-8 h-8 text-dark-800 dark:text-primary-600" title="Cargar Documento"
                                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                        class="size-6">
                                        <path fill-rule="evenodd"
                                            d="M12 2.25a.75.75 0 0 1 .75.75v11.69l3.22-3.22a.75.75 0 1 1 1.06 1.06l-4.5 4.5a.75.75 0 0 1-1.06 0l-4.5-4.5a.75.75 0 1 1 1.06-1.06l3.22 3.22V3a.75.75 0 0 1 .75-.75Zm-9 13.5a.75.75 0 0 1 .75.75v2.25a1.5 1.5 0 0 0 1.5 1.5h13.5a1.5 1.5 0 0 0 1.5-1.5V16.5a.75.75 0 0 1 1.5 0v2.25a3 3 0 0 1-3 3H5.25a3 3 0 0 1-3-3V16.5a.75.75 0 0 1 .75-.75Z"
        ...
        <!-- Modal de contacto -->
        <div id="contactModal"
            class="fixed inset-0 z-50 hidden bg-dark-800 bg-opacity-75 flex items-center justify-center">
            <div
                ...
                <!-- Cerrar Modal -->
                <button id="closeContactModal" class="absolute top-2 right-2 text-xl font-bold text-dark-600">
                    &times;
                </button>
                ...
                <form id="contactForm">
                    <input type="hidden" id="recipient_id" name="recipient_id" value="{{$profile->user_id}}">
                    <!-- Ajusta recipient_id según el destino -->

                    <div class="mb-4">
                        <label for="message" class="block text-sm font-medium text-dark-700">Mensaje</label>
                        <textarea id="message" name="message" rows="4"
                            class="w-full p-2 mt-1 border border-dark-300 rounded-md" required></textarea>
                    </div>

                    <div class="flex justify-between">
                        <!-- Botón de enviar -->
                        <div class="text-right">
                            <button type="submit" class="btn-success bg-blue-500 text-white py-2 px-4 rounded-md">Enviar
                                Mensaje</button>
                        </div>

                        <!-- Botón de cancelar -->
                        <div class="text-left">
                            <button type="button" id="cancelContactModal"
                                class="bg-dark-300 text-dark-800 py-2 px-4 rounded-md">Cancelar</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div id="modal" class="fixed inset-0 z-50 hidden bg-dark-800 bg-opacity-75 flex items-center justify-center">
            <div
                ...
                <button id="closeModal" class=""></button>
            </div>
        </div>
        ...
        <div id="myModal"
            ...
            <div class="modal-content bg-white p-6 rounded-lg w-[90%] max-w-md relative">
                <!-- Botón para cerrar el modal -->
                <span id="closeModal" class="absolute top-2 right-2 text-2xl cursor-pointer text-dark-700">×</span>
                <h2 class="text-lg font-semibold">Este es el título del Modal</h2>
                <p>Contenido del modal va aquí. Puedes agregar más detalles o elementos.</p>
            </div>
        </div>
        ...
        <div id="educationModal"
            ...
            <div
                ...
                <button id="closeEducationModal"
                    class="mt-4 bg-primary-600 text-white p-2 rounded-full w-full">Cerrar</button>
            </div>
        </div>
            ...
            <div
                ...
                <div id="experienceContent" class="mb-4">
                    ...
                    <div class="bg-white shadow-lg rounded-lg p-6 cursor-pointer work-experience transition-transform duration-300 ease-in-out transform hover:scale-105 hover:shadow-2xl hover:ring hover:ring-blue-300"
                        data-id="{{ $workExperience->id }}" data-job-title="{{ $workExperience->job_title }}"
                        data-company="{{ $workExperience->company }}"
                        data-start-date="{{ $workExperience->start_date }}"
                        data-end-date="{{ $workExperience->end_date }}">

                        <h3 class="text-xl font-bold text-dark-800">Cargo: {{ $workExperience->job_title }}</h3>
                        <p class="text-dark-600 text-md">Empresa: {{ $workExperience->company }}</p>
                        <p class="text-dark-500 text-sm">Inicio: {{
                            \Carbon\Carbon::parse($workExperience->start_date)->format('d/m/Y') }}</p>
                        <p class="text-dark-500 text-sm">
                            Finalización: {{ $workExperience->end_date ?
                            \Carbon\Carbon::parse($workExperience->end_date)->format('d/m/Y') : 'Actualidad' }}
                        </p>
                    </div>
                    @endforeach
                    <button id="closeExperienceModal"
                        class="mt-4 bg-primary-600 text-white p-2 rounded-full w-full">Cerrar</button>
                </div>
            </div>
