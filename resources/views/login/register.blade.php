<x-layout>
    <head>
        <meta name="preview-name-url" content="{{ route('previewName') }}">
        <meta name="preview-email-url" content="{{ route('previewEmail') }}">
        <meta name="preview-password-url" content="{{ route('previewPassword') }}">
    ...
    <div class="text-dark-900 bg-light-100 dark:text-dark-100 dark:bg-dark-900 flex items-center justify-center h-screen">
        ...
        <div class="min-h-screen w-full flex items-center justify-center bg-cover bg-center relative" style="background-image: url('/img/red1.png');">
            <div class="absolute inset-0 bg-black opacity-80"></div>

            <div class="bg-dark-900/50 rounded-lg shadow-lg backdrop-blur-lg border border-dark-700 dark:shadow-[0_0_10px_theme(colors.primary.700/80%)] w-full max-w-7xl h-[750px] mx-auto p-4 flex flex-col md:flex-row">
                <!-- BLOQUE A -->
                <div class="flex flex-col justify-center items-center max-w-xl mx-auto">
                    <div class="flex flex-col items-center text-center p-6 pb-2 sm:pb-10">
                        <img src="img/Logo.png" alt="Logo" class="w-40 h-40 object-contain mb-1 filter drop-shadow-[0_0_10px_theme(colors.light.300/80%)]">
                        <div>
                    ...
                    <div class="text-center md:text-center p-6">
                        <h2 class="text-xl sm:text-4xl font-bold mb-4 bg-gradient-to-r from-[theme(colors.primary.600)] to-[theme(colors.complem.400)] text-transparent bg-clip-text leading-tight filter drop-shadow-[0_0_10px_theme(colors.dark.950/90%)]">
                ...
                <div class="flex-1 md:w-1/2 px-4 pb-6 pt-0 flex flex-col justify-between h-full">
                    ...
                    <div class="flex justify-end p-2 sm:pb-2">
                        <button id="theme-toggle" type="button" class="flex items-center text-dark-500 dark:text-dark-400 hover:text-dark-700 dark:hover:text-dark-300 focus:outline-none text-sm transition duration-300 ease-in-out transform hover:scale-105">
                    ...
                    <div class="flex justify-center items-center h-full">
                        <form action="{{ route('register') }}" method="POST" class="w-full max-w-xs p-4 bg-light-200 dark:bg-dark-900 rounded-lg shadow-lg border border-primary-800">
                            ...
                            <div class="mb-4">
                                ...
                                <p id="nameMessage" class="text-red-500 text-xs mt-1"></p>
                                @error('name')
                            ...
                            <div class="mb-4">
                                ...
                                <p id="emailMessage" class="text-red-500 text-xs mt-1"></p>
                                @error('email')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <!-- Error dinámico con AJAX -->
                            <div id="password_error" class="text-xs text-red-500 mb-2"></div>
                            ...
                            <p class="mt-4 text-xs text-center text-dark-600 dark:text-dark-300">
                                ¿Ya estás registrado?
                                <a href="{{ url('/login') }}" class="text-primary-800 dark:text-primary-500 hover:underline">Inicia sesión</a>
                            </p>
                        </form>
                    </div>
                </div>
    ...
    <script src="{{ asset('js/registerJS/validateUserName.js') }}"></script>
    <script src="{{ asset('js/registerJS/validateEmail.js') }}"></script>
    <script src="{{ asset('js/registerJS/validatePassword.js') }}"></script>
</x-layout>
